...
class EndpointConnectionManagerTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun simulcastConfigIsSet() = runTest {
        val videoParameters = VideoParameters.presetFHD169.copy(
        ...
        every { endpointConnectionMock.senders } returns listOf(
            ...
            mockk(relaxed = true) {
                every { parameters } returns mockk(relaxed = true) {
                    ...
                    every { getEncodings() } returns listOf(
                        h,
                        m,
                        l
                    )
